...
AUTHZ-08/AUTHZ-09/SVC-01(K8s NetworkPolicy 기반)은 ansible/roles/auth_authz에 별도
구현되어 있다(kubernetes.core.k8s_info 사용, 파드 쉘 진입 없음 원칙과 무관하게
mgmt 서버에서 K8s API로 직접 조회하는 게 자연스러워 Ansible 쪽에 둠).
"""
import json
import re
...
def check_svc_02_vpc_cni(eks, cluster_names):
    """[확정 — 2026-08-19 인프라 확정] vpc-cni >= 1.14, enableNetworkPolicy = true."""
...
def check_authn_14_tls_enforced(elbv2):
    """[확정 — 2026-09-15 축소실행본 v1.0 신규 편입] ALB에 평문(HTTP) 트래픽을 실제로
    처리하는 리스너가 없어야 함(80번이 없거나 443 리다이렉트 전용이면 PASS)."""
