...
def check_4_10_s3_access_logging(s3, log_buckets, detect_errors):
    # [2026-09-24 보안팀 판단(사용자 확정)] 원문 판단기준은 "로그를 보관하고 있는 버킷"의
    # 서버 액세스 로깅 설정 여부 — 전체 버킷이 아니라 자동 탐지한 로그 보관 버킷만 판정한다.
    # 다른 버킷의 액세스 로그만 받는 수신 전용 버킷은 로그 보관 버킷으로 탐지되지 않으므로
    # 자연히 대상에서 빠진다(수신 버킷까지 로깅을 요구하면 체인이 끝나지 않는 문제 방지).
    # 일부 출처 탐지에 실패했는데 FAIL이 없으면, 놓친 버킷이 있을 수 있어 PASS 대신 REVIEW.
    item = "S3 버킷 로깅 설정"
