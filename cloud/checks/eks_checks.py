...
def check_1_12_automount_token(cluster_name):
    ...
    # 2026-09-24 — 판정 범위를 default SA + 앱 네임스페이스 SA로 한정(config.EKS_APP_NAMESPACES 주석 참고)
    violations = [
...
def check_2_3_service_policies(iam, cluster_names):
    # [2026-09-22] 서비스별 전용 SA(config.SERVICE_IAM_POLICY_MAP.service_accounts)로
    # 분리 완료 — 서비스마다 개별 IRSA role을 조회해서 S3/SecretManager 과잉권한(전원
    # 미보유가 baseline) + KMS(SERVICE_IAM_KMS_CHECK_ENABLED일 때만, auth-service만
    # SERVICE_IAM_KMS_ALLOWED_ACTIONS 보유·나머지는 미보유가 baseline)를 대조한다.
