...
def check_3_2_sg_unnecessary_rules(ec2):
    if config.SG_ALLOWED_RULES_WHITELIST is not None:
        return [make_result("3.2", "보안 그룹 인/아웃바운드 불필요 정책 관리", "REVIEW",
                             "화이트리스트 존재 — 실제 규칙과 대조 필요")]
    sgs, err = safe_call(ec2.describe_security_groups)
    count = len(sgs["SecurityGroups"]) if not err else 0
    return [make_result("3.2", "보안 그룹 인/아웃바운드 불필요 정책 관리", "SKIP",
                         f"필요 규칙 화이트리스트 미확정(전체 SG {count}개, 참고용)")]
...
def check_3_6_nat_management(ec2):
    ...
    for inst in nat_instances:
        ...
        if not aerr and attr["SourceDestCheck"]["Value"] is True:
            offenders.append(inst["InstanceId"])
    status = "PASS" if nat_instances and not offenders else ("FAIL" if offenders else "REVIEW")
    detail = (
        f"NAT 인스턴스(Name 태그 'nat' 포함) {len(nat_instances)}대 중 "
        f"source/dest check 활성화(비정상): {', '.join(offenders) if offenders else '없음'}"
        " — '목적 확인된 리소스 목록' 대조는 별도 확인 필요(config.NAT_PURPOSE_CONFIRMED_RESOURCES TODO)"
    )
